...
engine cannot then score that engine, because that would measure agreement
rather than accuracy. `harness/reference.py` refuses that combination outright and a test
asserts the refusal.
