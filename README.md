...
**It published a failing result for most of this run** — 2 of 13 lines
unreconciled — because that was what the deployed build actually produced, and a
...
projection that a 0.04 % balance change could flip. **It now passes: 12 lines, 0
unreconciled**, and it is a *gate* rather than a document — `tools/alert_audit_doc.py`
exits non-zero on any unreconciled line, so it cannot drift back into agreeing
with itself.
