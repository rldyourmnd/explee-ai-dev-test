...
A `depleted_at` mismatch also appeared and was fixed —
...
|---|---|---|
| [1 — Spend observability](task1-spend-observability/) | `monitor.py`, `alerts.jsonl`, public dashboard | collector gap-free since T0 — max consecutive gap under the 30 s poll interval, so the series has no hole; dashboard live at [spend.nddev.it.com](https://spend.nddev.it.com/) — HTTP 200, valid certificate, no login, verified externally |
